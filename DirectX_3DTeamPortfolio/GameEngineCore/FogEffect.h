...
struct FogInfo 
{
	...
	float4 FogColor = { 0.02f, 0.01f, 0.01f, 1.0f }; // 원하는 포그의 색상
	float FogMinHeight = -2800.0f; // 포그가 시작되는 거리
	float FogMaxHeight = -2400.0f; // 포그가 오브젝트를 완전히 가리는 거리 
	float NoiseScale = 0.1f; // 노이즈 텍스처 좌표의 스케일
