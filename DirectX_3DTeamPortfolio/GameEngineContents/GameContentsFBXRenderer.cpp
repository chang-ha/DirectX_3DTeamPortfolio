	...
	if (false == ParentRenderer->Pause)
	{
		...
		while (CurFrameTime >= Inter)
		{
			...
			if (true == RootMotion && nullptr != ParentRenderer->RootMotionComponent)
			{
				...
				MotionVector.X *= 100.f;
				MotionVector.Y *= 100.f;
				MotionVector.Z *= 100.f;
				...
				ParentRenderer->RootMotionComponent->MoveForce(physx::PxVec3(MotionVector.X, MotionVector.Y, MotionVector.Z), true);
